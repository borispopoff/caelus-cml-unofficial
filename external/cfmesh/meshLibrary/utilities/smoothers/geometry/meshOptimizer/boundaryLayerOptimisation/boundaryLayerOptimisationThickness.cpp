...
) const
{
    ...
    const scalar alpha =
        ...
        (
            ...
            (
                -1.0,
                CML::min(1.0, cosAlpha / (currThickness + VSMALL))
            )
        );
    ...
    const scalar beta =
        ...
        (
            ...
            (
                -1.0,
                CML::min(1.0, cosBeta / (currNeiThickness + VSMALL))
            )
        );
    ...
    if( (alpha + beta) < M_PI )
    {
        ...
        const scalar sinGamma = CML::max(SMALL, CML::sin(gamma));
        const scalar sinAlpha = CML::max(SMALL, CML::sin(alpha));
        const scalar sinBeta = CML::max(SMALL, CML::sin(beta));
...
) const
{
    ...
    forAll(c, fI)
    {
        ...
        if( help::shareAnEdge(bf, f) && (f.which(he.start()) == label(-1)) )
        {
            point intersection;
...
)
{
    ...
    forAll(hairEdges_, hairEdgeI)
    {
        ...
        hairLength[hairEdgeI] = (CML::mag(n) + VSMALL);
                    ...
                    forAll(bf, pI)
                    {
                        ...
                        forAllRow(hairEdgesAtBndPoint_, bpJ, peJ)
                        {
                            ...
                            if( nhe == hairEdges_[heJ] )
                            {
                                ...
                                maxThickness =
                                    CML::min(maxThickness, edgeThickness);
                            }
                        }
                    }
                }
