...
)
{
    ...
    forAll(hairEdgeType_, heI)
    {
        ...
        forAll(patchesAtPoint, ptchI)
        {
            ...
            bpToSimplex.insert(bpI, label(0));
            ...
            forAllRow(pTriangles, triPointI, ptI)
            {
                ...
                if( tri.region() == patchI )
                {
                    ...
                    tets.append
                    (
                        partTet(triLabels[2], triLabels[1], triLabels[0], label(1))
                    );
                }
            }

            //- create partTeMeshSimplex
            partTetMeshSimplex simplex(pts, tets, label(1));
