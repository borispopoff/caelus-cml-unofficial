...
void meshUntangler::cutRegion::findNewFaces()
{
    ...
    do
    {
        ...
        forAll(fcs, fI)
        {
            ...
            if( newFace.size() > label(1) )
            {
                DynList<label, 4> newEdge;
                forAll(pointUsage, pI)
                    if( pointUsage[pI] == label(1) )
                        newEdge.append(pI);

                if( newEdge.size() == label(2) )
                {
                    # ifdef DEBUGSmooth
        ...
        forAll(edgeUsage, eI)
            if( edgeUsage[eI] == label(1) )
                faceInPlane.append(eI);

        if( faceInPlane.size() > label(2) )
        {
            # ifdef DEBUGSmooth
        ...
        if( eFaces.size() > label(5) )
        forAll(eFaces, fI)
            if( eFaces[fI].size() != label(2) )
            {
