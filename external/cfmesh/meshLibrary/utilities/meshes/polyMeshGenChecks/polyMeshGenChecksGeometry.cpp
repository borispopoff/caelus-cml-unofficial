...
)
{
    ...
    # endif
    {
        ...
        forAll(faceAreas, facei)
        {
            ...
            localMinArea = CML::min(localMinArea, magFaceArea);
            localMaxArea = CML::max(localMaxArea, magFaceArea);
        }
        ...
        # endif
        {
            minArea = CML::min(minArea, localMinArea);
            maxArea = CML::max(maxArea, localMaxArea);
        }
    }
...
)
{
    ...
    # endif
    {
        ...
        for(label facei=0;facei<nInternalFaces;++facei)
        {
            ...
            localMinDDotS = CML::min(dDotS, localMinDDotS);
            sumDDotS += dDotS;
        }
        ...
        # pragma omp critical
        # endif
        minDDotS = CML::min(minDDotS, localMinDDotS);
    }
...
)
{
    ...
    # endif
    {
        ...
        forAll(faceSkewness, facei)
        {
            ...
            localMaxSkew = CML::max(localMaxSkew, skewness);
            sumSkew += skewness;
        }
        ...
        # pragma omp critical
        # endif
        maxSkew = CML::max(maxSkew, localMaxSkew);
    }
...
)
{
    ...
    for(label facei=0;facei<nInternalFaces;++facei)
    {
        ...
        const scalar dOwn
        (
            CML::mag(centres[owner[facei]] - fCentres[facei])
        );
        const scalar dNei
        (
            CML::mag(centres[neighbour[facei]] - fCentres[facei])
        );

        faceUniformity[facei] = CML::min(dOwn, dNei) / (dOwn + dNei);
    }
    ...
    if( Pstream::parRun() )
    {
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            forAll(dst, facei)
            {
                ...
                dst[facei] = CML::mag(centres[owner[fI]] - fCentres[fI]);
            }
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            forAll(otherDst, facei)
            {
                const label fI = start + facei;
                const scalar dOwn =
                    CML::mag(centres[owner[fI]] - fCentres[fI]);
                const scalar dNei = otherDst[facei];
                faceUniformity[fI] = CML::min(dOwn, dNei) / (dOwn + dNei);
            }
        }
    }
}
...
)
{
    ...
    # endif
    {
        ...
        for(label facei=0;facei<nInternalFaces;++facei)
        {
            ...
            localMaxUniformity = CML::max(localMaxUniformity, uniformity);
            localMinUniformity = CML::min(localMinUniformity, uniformity);
        ...
        # endif
        {
            maxUniformity = CML::max(maxUniformity, localMaxUniformity);
            minUniformity = CML::min(minUniformity, localMinUniformity);
        }
    }
    ...
    if( Pstream::parRun() )
    {
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            for(label fI=0;fI<size;++fI)
            {
                ...
                maxUniformity = CML::max(maxUniformity, uniformity);
                minUniformity = CML::min(minUniformity, uniformity);
...
)
{
    ...
    const scalar maxSin = CML::sin(maxDeg/180.0*M_PI);
    ...
    # endif
    {
                    ...
                    else
                    {
                        ...
                        if( (edgeNormal & faceNormals[facei]) < SMALL )
                        {
                            ...
                            # endif
                            {
                                ...
                                localMaxEdgeSin =
                                    CML::max(localMaxEdgeSin, magEdgeNormal);
                            }
                        }
                    }
                }
        ...
        # pragma omp critical
        # endif
        maxEdgeSin = CML::max(maxEdgeSin, localMaxEdgeSin);
    }
    ...
    if( report )
    {
        if( maxEdgeSin > SMALL )
        {
            scalar maxConcaveDegr =
                CML::asin(CML::min(1.0, maxEdgeSin))
             * 180.0/M_PI;
    ...
    # endif
    {
        ...
        forAll(faces, facei)
        {
            ...
            if( f.size() > 3 && magAreas[facei] > VSMALL )
            {
                ...
                minFlatnessProc = CML::min(minFlatnessProc, flatness);
        ...
        # endif
        {
            minFlatness = CML::min(minFlatness, minFlatnessProc);
        }
    }
...
)
{
    ...
    Info << "Worst non-orthogonality " << CML::acos(minNonOrtho) * 180.0 / M_PI
         << " selecting faces with non-orthogonality greater than "
         << (CML::acos(warnNonOrtho) * 180.0 / M_PI) << endl;
