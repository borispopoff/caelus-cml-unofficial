...
void polyMeshGen2DEngine::findZMinPoints() const
{
    ...
    {
        if( CML::mag(points[pointI].z() - bb_.min().z()) < tZ )
        {
...
void polyMeshGen2DEngine::findZMaxPoints() const
{
    ...
    forAll(points, pointI)
    {
        if( CML::mag(points[pointI].z() - bb_.max().z()) < tZ )
        {
    ...
    zMaxToZMinPtr_(nullptr)
{
    ...
    # endif
    {
        ...
        forAll(points, pointI)
        {
            localMin = CML::min(localMin, points[pointI]);
            localMax = CML::max(localMax, points[pointI]);
        }
        ...
        # endif
        {
            bb_.min() = CML::min(bb_.min(), localMin);
            bb_.max() = CML::max(bb_.max(), localMax);
        }
    }
