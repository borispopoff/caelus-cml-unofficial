...
#include "findCellsIntersectingSurface.hpp"
...
void findCellsIntersectingSurface::findIntersectedCells()
{
    ...
    forAll(cells, celli)
    {
        ...
        forAll(c, fI)
        {
            ...
            forAll(f, pI)
            {
                bb.min() = CML::min(bb.min(), points[f[pI]]);
                bb.max() = CML::max(bb.max(), points[f[pI]]);
            }
        }
        ...
        forAllConstIter(labelHashSet, triangles, tIter)
        {
            ...
            for(label i=1;i<3;++i)
            {
                ...
                obb.min() = CML::min(obb.min(), v);
                obb.max() = CML::max(obb.max(), v);
            }
