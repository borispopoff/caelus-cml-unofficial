...
#ifndef helperFunctionsFrontalMarking_HPP
#define helperFunctionsFrontalMarking_HPP
...
)
{
    ...
    DynList<label> nGroupsAtThread(nThreads, label(0));
    ...
    # endif
    {
        const label chunkSize =
            CML::max(1, neighbourCalculator.size() / nThreads);
        ...
        if( threadI == (nThreads - 1) )
            maxEl = CML::max(maxEl, neighbourCalculator.size());
    ...
    {
        if( globalGroupLabel[groupI] != label(-1) )
            continue;
