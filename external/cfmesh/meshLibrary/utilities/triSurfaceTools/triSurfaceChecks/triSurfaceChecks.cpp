...
)
{
    ...
    const scalar tol = CML::cos(angleTol * M_PI / 180.0);
...
{
    bb.min() = CML::min(surf.points());
    bb.max() = CML::max(surf.points());
}
...
)
{
    ...
    forAll(intersected, tI)
    {
        ...
        for(label i=1;i<3;++i)
        {
            bb.min() = CML::min(bb.min(), pts[tri[i]]);
            bb.max() = CML::max(bb.max(), pts[tri[i]]);
        }
...
)
{
    ...
    const scalar cosVal = CML::cos(angleTol * M_PI / 180.0);
