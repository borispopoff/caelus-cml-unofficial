    ...
    # endif
    {
        ...
        forAll(pointEdges, pI)
        {
            ...
            if( features.size() == 2 )
            {
                ...
                const scalar curv = CML::acos(cs) / (0.5 * (d1+d2+VSMALL));
                edgePointCurvature_[pI] = CML::mag(curv);
            }
        }
    }
}
...
void triSurfaceCurvatureEstimator::calculateGaussianCurvature()
{
    ...
    forAll(surface_, triI)
    {
        ...
        forAll(tri, pI)
        {
            ...
            gaussianCurvature_[region][tri[pI]] -= CML::acos(-1.0 * cs);
    ...
    forAll(meanCurvature_, patchI)
    {
        ...
        for(std::map<label, scalar>::iterator it=gc.begin();it!=gc.end();++it)
        {
            ...
            forAll(pEdges, peI)
            {
                ...
                {
                    maxCurv = CML::max(maxCurv, gc[ovI]);
                    minCurv = CML::min(minCurv, gc[ovI]);
                    ++nNei;
                }
            }
...
void triSurfaceCurvatureEstimator::calculateMeanCurvature()
{
    ...
    forAll(edges, eI)
    {
        ...
        {
            //- calculate the angle
            const scalar angle = CML::atan2(sn, cs);
            Hf += angle * length;
        }
    ...
    forAll(meanCurvature_, patchI)
    {
        ...
        for(std::map<label, scalar>::iterator it=mc.begin();it!=mc.end();++it)
        {
            ...
            forAll(pEdges, peI)
            {
                ...
                {
                    maxCurv = CML::max(maxCurv, mc[ovI]);
                    minCurv = CML::min(minCurv, mc[ovI]);
                    ++nNei;
                }
            }
