...
#include "meshOctreeCubeCoordinatesScalar.hpp"
...
) const
{
    ...
    label maxLevel = CML::max(nLayers);
    ...
    forAllReverse(leavesForLayer, layerI)
    {
        ...
        # endif
        {
            ...
            # endif
            forAll(activeLeaves, i)
                localMax = CML::max(localMax, targetRefLevel[activeLeaves[i]]);

            # ifdef USE_OMP
            # pragma omp critical
            # endif
            maxLevel = CML::max(localMax, maxLevel);
        }
...
)
{
    ...
    # pragma omp parallel
    {
        ...
        {
            if( refineBox[leafI] )
                localMax = CML::max(localMax, leaves[leafI]->level());
        }

        # pragma omp critical
        maxLevel = CML::max(maxLevel, localMax);
    }
    # else
    forAll(refineBox, leafI)
    {
        if( refineBox[leafI] )
            maxLevel = CML::max(maxLevel, leaves[leafI]->level());
    }
    # endif
    ...
    forAllReverse(leavesForLevel, levelI)
    {
        ...
        # pragma omp parallel
        {
            ...
            forAll(activeLeaves, i)
            {
                ...
                nLayersForActive[i] = CML::max(ceil(refThickness[leafI]/cs), 1);

                localMaxNLayers =
                    CML::max(localMaxNLayers, nLayersForActive[i]);
            }

            # pragma omp critical
            maxNLayers = CML::max(maxNLayers, localMaxNLayers);
        }
        # else
        forAll(activeLeaves, i)
        {
            ...
            maxNLayers = CML::max(maxNLayers, nLayersForActive[i]);
        }
        # endif
