...
#include "objectRefinementList.hpp"
...
#include "edgeMeshGeometryModification.hpp"
        ...
        forAll(leaves, leafI)
        {
            ...
            forAll(refObjects, oI)
            {
                if( refObjects[oI].intersectsObject(bb) )
                {
                    ...
                    {
                        rThickness[leafI] =
                            CML::max(rThickness[leafI], refThickness[oI]);
    ...
    do
    {
        ...
        forAll(surfaceMeshesPtr, surfI)
        {
            ...
            forAll(surf, triI)
            {
                ...
                {
                    triBB.min() = CML::min(triBB.min(), points[tri[pI]]);
                    triBB.max() = CML::max(triBB.max(), points[tri[pI]]);
                }
            ...
            forAll(edges, edgeI)
            {
                ...
                if( refThickness[emI] > VSMALL )
                {
                    ...
                    forAll(intersectedLeaves, i)
                    {
                        ...
                        rThickness[leafI] =
                            CML::max(rThickness[leafI], refThickness[emI]);
                    }
                }
            }
        }
