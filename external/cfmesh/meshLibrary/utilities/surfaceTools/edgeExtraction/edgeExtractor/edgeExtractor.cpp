...
#include "meshSurfaceCheckInvertedVertices.hpp"
#include "meshSurfaceCheckEdgeTypes.hpp"
...
void edgeExtractor::findPatchesNearSurfaceFace()
{
    ...
    # endif
    {
        ...
        forAll(bFaces, bfI)
        {
            ...
            scalar d(0.0);
            forAll(bf, pI)
                d = CML::max(d, CML::mag(c - points[bf[pI]]));
            d = 2.0 * d + VSMALL;
    ...
    do
    {
        ...
        forAll(edgeType_, beI)
        {
            ...
            forAll(c, fI)
            {
                ...
                forAll(f, pI)
                {
                    pMin = CML::min(pMin, points[f[pI]]);
                    pMax = CML::max(pMax, points[f[pI]]);
                }
            }
