...
void edgeExtractor::faceEvaluator::calculateNeiPatchesParallelNewPatches()
{
    ...
    if( Pstream::parRun() )
    {
        ...
        forAllConstIter(Map<label>, globalToLocal, it)
        {
            ...
            if( edgeFaces.sizeOfRow(beI) == label(1) )
            {
                labelLongList& dts = exchangeData[otherProc[beI]];
                //- send data as follows:
                //- 1. global edge label
...
) const
{
    ...
    forAllRow(faceEdges, bfI, feI)
    {
        ...
        if( edgeFaces.sizeOfRow(beI) == label(2) )
        {
...
) const
{
    ...
    forAllRow(faceEdges, bfI, feI)
    {
        ...
        if( edgeFaces.sizeOfRow(beI) == label(2) )
        {
            label nei = edgeFaces(beI, label(0));
            if( nei == bfI )
                nei = edgeFaces(beI, label(1));

            neiPatches[feI] = fPatches[nei];
        }
        else if( Pstream::parRun() && (edgeFaces.sizeOfRow(beI) == label(1)) )
        {
...
bool edgeExtractor::findCornerCandidates()
{
    ...
    forAll(bFaces, bfI)
    {
        ...
        forAll(bf, pI)
        {
            const scalar d = 2.0 * CML::mag(c - points[bf[pI]]);
            const label bpI = bp[bf[pI]];

            searchRange[bpI] = CML::max(d, searchRange[bpI]);
        }
    }
    ...
    if( Pstream::parRun() )
    {
        ...
        forAll(receivedData, i)
        {
            ...
            searchRange[bpI] =
                CML::max(searchRange[bpI], receivedData[i].value());
        }
    }
    ...
    forAll(bPoints, bpI)
    {
        ...
        forAll(containedTriangles, i)
        {
            ...
            forAll(tri, pI)
            {
                ...
                const scalar d = CML::magSqr(sp[spI] - points[bPoints[bpI]]);
...
bool edgeExtractor::checkCorners()
{
    ...
    do
    {
        ...
        forAllConstIter(labelHashSet, featureEdge, it)
        {
            ...
            (
                CML::min(patches[0], patches[1]),
                CML::max(patches[0], patches[1])
            );
