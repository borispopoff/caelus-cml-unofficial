    ...
    forAll(treatPatchesWithPatch_, patchI)
    {
        ...
        forAll(otherPatches, patchJ)
        {
            ...
            (
                std::make_pair
                (
                    CML::min(patchI, otherPatches[patchJ]),
                    CML::max(patchI, otherPatches[patchJ])
                )
            );
        }
    }
    ...
    do
    {
        ...
        forAll(bFaces, bfI)
        {
            ...
            forAll(neiPatches, eI)
            {
                ...
                (
                    CML::min(fPatch, neiPatches[eI]),
                    CML::max(fPatch, neiPatches[eI])
                );
                ...
                else
                {
                    ...
                    (
                        CML::min(fPatch, neiPatches[neiPatches.rcIndex(eI)]),
                        CML::max(fPatch, neiPatches[neiPatches.rcIndex(eI)])
                    );
                ...
                else
                {
                    ...
                    (
                        CML::min(fPatch, neiPatches[neiPatches.fcIndex(eI)]),
                        CML::max(fPatch, neiPatches[neiPatches.fcIndex(eI)])
                    );
