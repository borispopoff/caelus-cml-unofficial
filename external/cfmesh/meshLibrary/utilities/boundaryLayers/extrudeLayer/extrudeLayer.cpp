    ...
    if( Pstream::parRun() )
    {
        ...
        forAllConstIter(dualEdgesMap, procPointsDual, dIter)
        {
            ...
            forAll(dEdges, eI)
            {
                ...
                while( front.size() != label(0) )
                {
                    const label eLabel = front.removeLastElement();

                    forAll(dEdges, eJ)
                    {
                        if( edgeGroup[eJ] != label(-1) )
                            continue;
                        if( eJ == eLabel )
                            continue;

                        if( dEdges[eLabel].commonVertex(dEdges[eJ]) != label(-1) )
                        {
                            front.append(eJ);
                            edgeGroup[eJ] = group;
                        }
    ...
    forAll(pointFaces, pointI)
    {
        ...
        forAllRow(pointFaces, pointI, pfI)
        {
            ...
            while( front.size() )
            {
                ...
                {
                    if( faceGroup[pfJ] != label(-1) )
                        continue;

                    const label faceJ = pointFaces(pointI, pfJ);

                    if( owner[faceJ] < label(0) )
                        continue;
                ...
                forAllRow(pointFaces, pointI, pfI)
                {
                    ...
                    if( facei >= nOrigFaces_ )
                    {
                        ...
                        if( thickness_ < 0.0 )
                        {
                            ...
                            forAll(f, pI)
                                d = CML::min(d, CML::mag(points[f[pI]] - c));

                            d *= 0.4;
                            lps.scalarValue() = CML::min(lps.scalarValue(), d);
                        }
                        else
                        {
