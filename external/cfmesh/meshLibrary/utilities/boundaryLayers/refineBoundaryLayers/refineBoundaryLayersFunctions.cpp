...
#include "detectBoundaryLayers.hpp"

#include "labelledPair.hpp"
        ...
        forAll(layerPatches, lpI)
        {
            ...
            if( it != numLayersForPatch_.end() )
            {
                ...
                else
                {
                    ...
                    maxNumLayers = CML::max(maxNumLayers, it->second);
                    hasLocalValue = true;
                }
            }
        }
    ...
    # endif
    {
        ...
        forAll(splitEdges_, seI)
        {
            ...
            forAllRow(pointFaces, bpI, pfI)
            {
                ...
                //- at this edge
                nLayers = CML::max(nLayers, nLayersAtBndFace_[bfI]);
                ...
                if( tIt != maxThicknessForPatch_.end() )
                {
                    ...
                    {
                        thickness = CML::min(thickness, tIt->second);
                    }
                    else
                    {
        ...
        forAll(splitEdges_, seI)
        {
            ...
            if( thicknessRatio[seI] > (1. + SMALL) )
            {
                firstThickness =
                    magv /
                    (
                        (1 - CML::pow(thicknessRatio[seI], nLayersAtEdge[seI]))
            ...
            firstThickness =
                CML::min
                (
                    CML::max(firstLayerThickness[seI], SMALL),
                    firstThickness
                );
            ...
            if( specialMode_ )
            {
                ...
                for(label i=1;i<nLayersAtEdge[seI]-1;++i)
                    t += firstThickness * CML::pow(thicknessRatio[seI], i);
            ...
            for(label pI=1;pI<nLayers;++pI)
            {
                ...
                param += firstThickness * CML::pow(thicknessRatio[seI], pI);
