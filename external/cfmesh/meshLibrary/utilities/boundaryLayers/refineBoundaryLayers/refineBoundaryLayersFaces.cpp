...
)
{
    ...
    if( (dir1 != label(-1)) && (dir0 == label(-1)) )
    {
        dir0 = dir1;
        dir0Edges = dir1Edges;
        dir1 = label(-1);
    }
    else if( (dir0 != label(-1)) && (dir1 != label(-1)) && (dir1 != f.fcIndex(dir0)) )
    {
    ...
    forAll(facePoints, i)
    {
        forAll(facePoints[i], j)
        {
            if( facePoints[i][j] < 0 )
            {
                ...
                const scalar u
                (
                    CML::mag(points[facePoints[i][0]] - v00) /
                ...
                const scalar v
                (
                    CML::mag(points[facePoints[0][j]] - v00) /
    ...
    for(label j=0;j<nLayersDir1;++j)
    {
        for(label i=0;i<nLayersDir0;++i)
        {
            ...
            if( (dir1 != label(-1)) && (i == label(0)) && (j == (nLayersDir1 - 1)) )
            {
                # ifdef DEBUGLayer
            ...
            else
            {
                if( j == label(0) )
                {
                    forAllRow(newFaces_, nfI, pI)
                        if( f.which(newFaces_(nfI, pI)) >= label(0) )
                        {
...
void refineBoundaryLayers::generateNewFaces()
{
    ...
    forAll(bFaces, bfI)
    {
        ...
        forAll(bf, eI)
        {
            ...
            if( foundSame || (neiLayers.size() == label(0)) )
                continue;
                ...
                forAll(currentSplits, i)
                {
                    if( currentSplits[i].first() == dir )
                        currentSplits[i].second() =
                            CML::max(currentSplits[i].second(), nSplits);
                }
            }
        }
