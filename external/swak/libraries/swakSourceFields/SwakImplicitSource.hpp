...
template<class T>
CML::fv::SwakImplicitSource<T>::SwakImplicitSource
(
    const word& name,
    const word& modelType,
    const dictionary& dict,
...
//- Add implicit contribution to equation
template<class T>
void CML::fv::SwakImplicitSource<T>::addSup(fvMatrix<T>& eqn, const label fieldI)
{
    if (debug)
    {
