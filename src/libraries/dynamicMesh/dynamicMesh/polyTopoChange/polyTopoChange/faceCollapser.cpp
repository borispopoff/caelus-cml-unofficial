...
CML::label CML::faceCollapser::findEdge
(
...
)
{
    ...
    FatalErrorInFunction << "Cannot find edge between vertices " << v0
...
// Construct from mesh
CML::faceCollapser::faceCollapser(const polyMesh& mesh)
:
    mesh_(mesh)
{}
...
void CML::faceCollapser::setRefinement
(
    ...
    forAll(faceLabels, i)
    {
        ...
        while (fp != fpB)
        {
            ...
            if (w <= dist[fpMin1])
            {
                ...
                point newPoint
                (
                    pA + CML::sqrt(w / (dist[fpB] - dist[fpA]))*(pB - pA)
                );
