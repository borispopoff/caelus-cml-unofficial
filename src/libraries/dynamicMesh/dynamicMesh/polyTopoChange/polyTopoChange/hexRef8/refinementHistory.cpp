...
CML::refinementHistory::splitCell8::splitCell8(Istream& is)
{
    is >> *this;
}
...
CML::refinementHistory::splitCell8::splitCell8(const splitCell8& sc)
:
    parent_(sc.parent_),
    addedCellsPtr_
    (
...
bool CML::refinementHistory::splitCell8::operator==(const splitCell8& s) const
{
...
CML::Istream& CML::operator>>(Istream& is, refinementHistory::splitCell8& sc)
{
...
CML::label CML::refinementHistory::allocateSplitCell
(
    const label parent,
    const label i
)
...
void CML::refinementHistory::freeSplitCell(const label index)
{
...
void CML::refinementHistory::markSplit
(
    const label index,
    labelList& oldToNew,
...
void CML::refinementHistory::mark
(
    const label val,
    const label index,
...
CML::label CML::refinementHistory::markCommonCells
(
...
void CML::refinementHistory::add
(
...
void CML::refinementHistory::apply
(
...
//- Read or construct
CML::refinementHistory::refinementHistory
(
    const IOobject& io,
    const List<splitCell8>& splitCells,
    const labelList& visibleCells,
...
CML::autoPtr<CML::refinementHistory> CML::refinementHistory::clone
(
    const IOobject& io,
...
void CML::refinementHistory::resize(const label size)
{
...
void CML::refinementHistory::updateMesh(const mapPolyMesh& map)
{
    if (active())
    {
...
void CML::refinementHistory::subset
(
...
void CML::refinementHistory::countProc
(
    const label index,
...
void CML::refinementHistory::distribute(const mapDistributePolyMesh& map)
{
    if (!active())
    {
        FatalErrorInFunction
...
void CML::refinementHistory::compact()
{
    if (debug)
    {
...
void CML::refinementHistory::writeDebug() const
{
...
bool CML::refinementHistory::read()
{
...
bool CML::refinementHistory::readData(Istream& is)
{
    is >> *this;
    return !is.bad();
}
...
bool CML::refinementHistory::writeData(Ostream& os) const
{
    os << *this;
...
CML::Istream& CML::operator>>(Istream& is, refinementHistory& rh)
{
...
CML::Ostream& CML::operator<<(Ostream& os, const refinementHistory& rh)
{
