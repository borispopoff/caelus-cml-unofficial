...
#include "refinementDistanceData.hpp"
...
CML::scalar CML::hexRef8::getLevel0EdgeLength() const
{
    ...
    forAll(typEdgeLenSqr, levelI)
    {
        ...
        if (lenSqr < GREAT2)
        {
            level0Size = CML::sqrt(lenSqr)*(1<<levelI);

            if (debug)
            {
                Pout<< "hexRef8::getLevel0EdgeLength() :"
                    << " For level:" << levelI
                    << " have edgeLen:" << CML::sqrt(lenSqr)
...
void CML::hexRef8::getFaceNeighbours
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const label facei,
...
CML::label CML::hexRef8::findMinLevel(const labelList& f) const
{
...
CML::label CML::hexRef8::findMaxLevel(const labelList& f) const
{
...
CML::label CML::hexRef8::countAnchors
(
    const labelList& f,
...
// splitPoint.
void CML::hexRef8::insertEdgeSplit
(
...
CML::label CML::hexRef8::storeMidPointInfo
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
...
void CML::hexRef8::walkFaceToMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label facei,
...
void CML::hexRef8::walkFaceFromMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label facei,
...
CML::label CML::hexRef8::faceConsistentRefinement
(
    const bool maxSet,
...
void CML::hexRef8::checkWantedRefinementLevels
(
...
void CML::hexRef8::setInstance(const fileName& inst)
{
    if (debug)
    {
...
void CML::hexRef8::collectLevelPoints
(
    const labelList& f,
    const label level,
...
CML::hexRef8::hexRef8(const polyMesh& mesh, const bool readHistory)
:
    mesh_(mesh),
    cellLevel_
    (
...
CML::labelList CML::hexRef8::consistentRefinement
(
    const labelList& cellsToRefine,
    const bool maxSet
) const
...
CML::labelList CML::hexRef8::consistentSlowRefinement
(
...
CML::labelList CML::hexRef8::consistentSlowRefinement2
(
    const label maxFaceDiff,
    const labelList& cellsToRefine,
    const labelList& facesToCheck
...
CML::labelListList CML::hexRef8::setRefinement
(
...
void CML::hexRef8::updateMesh(const mapPolyMesh& map)
{
...
void CML::hexRef8::distribute(const mapDistributePolyMesh& map)
{
    if (debug)
    {
...
void CML::hexRef8::checkMesh() const
{
...
void CML::hexRef8::checkRefinementLevels
(
    const label maxPointDiff,
    const labelList& pointsToCheck
) const
...
const CML::cellShapeList& CML::hexRef8::cellShapes() const
{
...
CML::labelList CML::hexRef8::getSplitPoints() const
{
    if (debug)
    {
        checkRefinementLevels(-1, labelList(0));
...
//void CML::hexRef8::markCellClusters
//(
//    const label maxLevel,
//    labelList& cluster
//) const
...
CML::labelList CML::hexRef8::consistentUnrefinement
(
...
void CML::hexRef8::setUnrefinement
(
