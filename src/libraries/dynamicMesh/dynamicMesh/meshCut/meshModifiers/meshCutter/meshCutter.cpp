...
bool CML::meshCutter::uses(const labelList& elems1, const labelList& elems2)
{
    forAll(elems1, elemI)
    {
...
CML::label CML::meshCutter::findCutCell
(
    const cellCuts& cuts,
...
//  none found.
CML::label CML::meshCutter::findInternalFacePoint
(
...
void CML::meshCutter::getFaceInfo
(
    const label facei,
    label& patchID,
    label& zoneID,
...
CML::face CML::meshCutter::addEdgeCutsToFace(const label facei) const
{
    const face& f = mesh().faces()[facei];
...
CML::meshCutter::meshCutter(const polyMesh& mesh)
:
    edgeVertex(mesh),
    addedCells_(),
    addedFaces_(),
...
CML::meshCutter::~meshCutter()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void CML::meshCutter::setRefinement
(
    const cellCuts& cuts,
    polyTopoChange& meshMod
)
...
void CML::meshCutter::updateMesh(const mapPolyMesh& morphMap)
{
