...
const CML::NamedEnum<CML::directions::directionType, 3>
    CML::directions::directionTypeNames_;


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

// For debugging
void CML::directions::writeOBJ(Ostream& os, const point& pt)
{
    os << "v " << pt.x() << ' ' << pt.y() << ' ' << pt.z() << endl;
}

// For debugging
void CML::directions::writeOBJ
(
    Ostream& os,
    const point& pt0,
    const point& pt1,
...
CML::vectorField CML::directions::propagateDirection
(
    const polyMesh& mesh,
    const bool useTopo,
    const polyPatch& pp,
