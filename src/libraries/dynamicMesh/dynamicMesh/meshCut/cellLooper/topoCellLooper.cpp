...
const CML::scalar CML::topoCellLooper::featureCos = CML::cos(degToRad(10.0));
...
//      startVert != -1
//
void CML::topoCellLooper::walkFace
(
...
// not.
CML::labelList CML::topoCellLooper::getSuperEdge
(
    const cellFeatures& features,
    const label facei,
    const label startEdgeI,
...
CML::label CML::topoCellLooper::getAlignedNonFeatureEdge
(
    const vector& refDir,
    const label celli,
    const cellFeatures& features
...
void CML::topoCellLooper::walkAcrossFace
(
    const cellFeatures& features,
    const label facei,
    const label startEdgeI,
...
void CML::topoCellLooper::walkSplitHex
(
    const label celli,
    const cellFeatures& features,
...
CML::topoCellLooper::topoCellLooper(const polyMesh& mesh)
:
    hexCellLooper(mesh)
{}
...
CML::topoCellLooper::~topoCellLooper()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

bool CML::topoCellLooper::cut
(
    const vector& refDir,
    const label celli,
