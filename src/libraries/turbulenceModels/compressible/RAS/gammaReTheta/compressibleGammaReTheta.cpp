...
scalar const gammaReTheta::tol_ = 1.0e-4;
int const gammaReTheta::maxIter_ = 100;
...
{
    scalar const c1 = sqrt(2.0*pTraits<scalar>::one);
    scalar const c2 = 2.193*pTraits<scalar>::one;
    volTensorField const gradU(fvc::grad(this->U_));
    return sqr(this->y_)*c1*mag(symm(gradU))/(c2*mu()/rho_*ReThetac());
}

tmp<volScalarField> gammaReTheta::Fturb() const
{
    scalar const c3 = 0.25*pTraits<scalar>::one;
    return exp(-pow4(c3*Rt()));
}

tmp<volScalarField> gammaReTheta::Freattach() const
{
    scalar const c4 = 0.05*pTraits<scalar>::one; 
    return exp(-pow4(c4*Rt()));
}

tmp<volScalarField> gammaReTheta::Fwake() const
{
    scalar const c5 = 1.0e05*pTraits<scalar>::one;
    return exp(-sqr(sqr(y_)*omega_/(c5*mu()/rho_)));
}

tmp<volScalarField> gammaReTheta::FThetat() const
{
    scalar const c6 = sqrt(2.0*pTraits<scalar>::one);
    volTensorField const gradU(fvc::grad(this->U_));
    ...
    scalar const c7 = 375*pTraits<scalar>::one;
    scalar const c8 = pTraits<scalar>::one;
...
{
    volScalarField const U2gradU((sqr(U_)&&(fvc::grad(U_))));
    ...
    forAll(ReThetaField, celli)
    {
        ...
        scalar const c12 = 0.1*pTraits<scalar>::one;
        do
        {
...
tmp<volScalarField> gammaReTheta::F1(volScalarField const& CDkOmega) const
{
    ...
    (
        tanh(pow4(arg1)),
	exp(-sqr(pow4(y_*sqrt(k_)/(scalar(120)*mu()/rho_))))
    );
}
...
tmp<volSymmTensorField> gammaReTheta::devRhoReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
void gammaReTheta::correct()
{
    ...
    (
        fvm::ddt(rho_,intermittency_)
      + fvm::div(phi_, intermittency_)
      - fvm::Sp(fvc::div(phi_), intermittency_)
      - fvm::laplacian(DintermittencyEff(), intermittency_)
     ==
        Flength()*ca1_*rho_*sqrt(scalar(2))*mag(symm(fvc::grad(U_)))*sqrt(Fonset()*intermittency_)
      - fvm::Sp
        (
         Flength()*ca1_*rho_*sqrt(scalar(2))*mag(symm(fvc::grad(U_)))*sqrt(Fonset()*intermittency_)*ce1_,
	    intermittency_
	)
      + ca2_*rho_*sqrt(scalar(2))*mag(skew(fvc::grad(U_)))*Fturb()*intermittency_
      - fvm::Sp
        (
