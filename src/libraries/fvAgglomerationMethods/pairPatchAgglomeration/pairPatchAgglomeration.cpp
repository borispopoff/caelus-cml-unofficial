...
#include "pairPatchAgglomeration.hpp"
#include "meshTools.hpp"
#include "unitConversion.hpp"

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void CML::pairPatchAgglomeration::compactLevels(const label nCreatedLevels)
{
...
bool CML::pairPatchAgglomeration::continueAgglomerating
(
...
void CML::pairPatchAgglomeration::setBasedEdgeWeights()
{
    ...
    forAll(coarsePatch.edges(), i)
    {
        if (coarsePatch.isInternalEdge(i))
        {
            ...
            if (eFaces.size() == 2)
            {
                ...
                if (cosI < CML::cos(degToRad(featureAngle_)))
                {
...
void CML::pairPatchAgglomeration::setEdgeWeights
(
...
CML::pairPatchAgglomeration::pairPatchAgglomeration
(
...
const CML::bPatch& CML::pairPatchAgglomeration::patchLevel
(
    const label i
) const
{
...
void CML::pairPatchAgglomeration::mapBaseToTopAgglom
(
    const label fineLevelIndex
)
{
...
bool CML::pairPatchAgglomeration::agglomeratePatch
(
    const bPatch& patch,
...
void CML::pairPatchAgglomeration::combineLevels(const label curLevel)
{
