...
void CML::ptscotchDecomp::check(const int retVal, const char* str)
{
    if (retVal)
    {
...
//CML::label CML::ptscotchDecomp::decomposeZeroDomains
//(
...
//
//    Field<label> xadj(initxadj);
//    Field<label> adjncy(initadjncy);
...
CML::label CML::ptscotchDecomp::decompose
(
    const fileName& meshPath,
...
) const
{
    ...
    if (decompositionDict_.found("scotchCoeffs"))
    {
        ...
        if (scotchCoeffs.lookupOrDefault("writeGraph", false))
        {
            OFstream str
            (
               meshPath + "_" + CML::name(Pstream::myProcNo()) + ".dgr"
            );
...
CML::ptscotchDecomp::ptscotchDecomp(const dictionary& decompositionDict)
:
...
CML::labelList CML::ptscotchDecomp::decompose
(
    const polyMesh& mesh,
