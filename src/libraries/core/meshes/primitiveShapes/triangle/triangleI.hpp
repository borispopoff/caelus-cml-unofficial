...
inline CML::triangle<Point, PointRef>::triangle
(
    const Point& a,
    const Point& b,
    const Point& c
...
template<class Point, class PointRef>
inline CML::triangle<Point, PointRef>::triangle(Istream& is)
{
    is  >> *this;
}
...
template<class Point, class PointRef>
inline const Point& CML::triangle<Point, PointRef>::a() const
{
    return a_;
}

template<class Point, class PointRef>
inline const Point& CML::triangle<Point, PointRef>::b() const
{
    return b_;
}

template<class Point, class PointRef>
inline const Point& CML::triangle<Point, PointRef>::c() const
{
    return c_;
}
...
template<class Point, class PointRef>
inline Point CML::triangle<Point, PointRef>::centre() const
{
...
template<class Point, class PointRef>
inline Point CML::triangle<Point, PointRef>::circumCentre() const
{
...
template<class Point, class PointRef>
inline CML::scalar CML::triangle<Point, PointRef>::circumRadius() const
{
    scalar d1 =  (c_ - a_) & (b_ - a_);
    scalar d2 = -(c_ - b_) & (b_ - a_);
    scalar d3 =  (c_ - a_) & (c_ - b_);
    ...
    else
    {
        ...
        return 0.5*CML::sqrt(min(GREAT, max(0, a)));
    }
}


template<class Point, class PointRef>
inline CML::scalar CML::triangle<Point, PointRef>::quality() const
{
    ...
    return mag()/(CML::sqr(c)*3.0*sqrt(3.0)/4.0);
}


template<class Point, class PointRef>
inline CML::scalar CML::triangle<Point, PointRef>::sweptVol
(
    const triangle& t
) const
{
...
template<class Point, class PointRef>
inline CML::tensor CML::triangle<Point, PointRef>::inertia
(
    PointRef refPt,
    scalar density
) const
...
template<class Point, class PointRef>
inline CML::pointHit CML::triangle<Point, PointRef>::ray
(
    const point& p,
    const vector& q,
...
) const
{
    ...
    if (magArea < VSMALL)
    {
        ...
        inter.setDistance(CML::mag(a_ - p));
...
// Moeller/Trumbore.
template<class Point, class PointRef>
inline CML::pointHit CML::triangle<Point, PointRef>::intersection
(
    const point& orig,
    const vector& dir,
    const intersection::algorithm alg,
...
template<class Point, class PointRef>
CML::pointHit CML::triangle<Point, PointRef>::nearestPointClassify
(
    const point& p,
    label& nearType,
    label& nearLabel
) const
{
    ...
    if (d1 <= 0.0 && d2 <= 0.0)
    {
        ...
        nearType = POINT;
        nearLabel = 0;
        return pointHit(false, a_, CML::mag(a_ - p), true);
    }
    ...
    if (d3 >= 0.0 && d4 <= d3)
    {
        ...
        nearType = POINT;
        nearLabel = 1;
        return pointHit(false, b_, CML::mag(b_ - p), true);
    }
    ...
    if (vc <= 0.0 && d1 >= 0.0 && d3 <= 0.0)
    {
        ...
        nearType = EDGE;
        nearLabel = 0;
        return pointHit(false, nearPt, CML::mag(nearPt - p), true);
    }
    ...
    if (d6 >= 0.0 && d5 <= d6)
    {
        ...
        nearType = POINT;
        nearLabel = 2;
        return pointHit(false, c_, CML::mag(c_ - p), true);
    }
    ...
    if ((va + vb + vc) < ROOTVSMALL)
    {
        ...
        nearType = NONE,
        nearLabel = -1;
        return pointHit(true, nearPt, CML::mag(nearPt - p), false);
    }
...
template<class Point, class PointRef>
inline bool CML::triangle<Point, PointRef>::classify
(
    const point& p,
    label& nearType,
    label& nearLabel
