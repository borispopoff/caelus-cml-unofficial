...
void CML::coupledPolyPatch::writeOBJ(Ostream& os, const point& pt)
{
    os << "v " << pt.x() << ' ' << pt.y() << ' ' << pt.z() << endl;
}


void CML::coupledPolyPatch::writeOBJ
(
    Ostream& os,
...
)
{
    ...
    Map<label> caelusToObj(4*faces.size());
    ...
    forAll(faces, i)
    {
        ...
        os << ' ' << caelusToObj[f[0]]+1 << nl;
    }
}


CML::pointField CML::coupledPolyPatch::getAnchorPoints
(
    const UList<face>& faces,
    const pointField& points,
...
CML::scalarField CML::coupledPolyPatch::calcFaceTol
(
    const UList<face>& faces,
    const pointField& points,
...
CML::label CML::coupledPolyPatch::getRotation
(
    const pointField& points,
    const face& f,
    const point& anchor,
...
void CML::coupledPolyPatch::calcTransformTensors
(
...
) const
{
    ...
    {
        scalar error = absTol*CML::sqrt(1.0*Cf.size());
...
CML::coupledPolyPatch::coupledPolyPatch
(
    const word& name,
    const label size,
    const label start,
