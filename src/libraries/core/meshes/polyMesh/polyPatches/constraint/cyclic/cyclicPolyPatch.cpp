...
CML::label CML::cyclicPolyPatch::findMaxArea
(
...
void CML::cyclicPolyPatch::calcTransforms()
{
    if (size())
    {
        // Half0
...
void CML::cyclicPolyPatch::getCentresAndAnchors
(
...
CML::vector CML::cyclicPolyPatch::findFaceMaxRadius
(
...
CML::cyclicPolyPatch::cyclicPolyPatch
(
    const word& name,
    const label size,
    const label start,
...
CML::label CML::cyclicPolyPatch::neighbPatchID() const
{
    if (neighbPatchID_ == -1)
    {
...
void CML::cyclicPolyPatch::transformPosition(pointField& l) const
{
    if (!parallel())
    {
        if (transform() == ROTATIONAL)
...
void CML::cyclicPolyPatch::transformPosition(point& l, const label facei) const
{
    if (!parallel())
    {
        const tensor& T =
...
void CML::cyclicPolyPatch::initGeometry(PstreamBuffers& pBufs)
{
    polyPatch::initGeometry(pBufs);
}


void CML::cyclicPolyPatch::initGeometry
(
...
void CML::cyclicPolyPatch::calcGeometry
(
    const primitivePatch& referPatch,
...
void CML::cyclicPolyPatch::calcGeometry(PstreamBuffers& pBufs)
{
    calcGeometry
    (
        *this,
...
void CML::cyclicPolyPatch::initMovePoints
(
...
void CML::cyclicPolyPatch::movePoints
(
    PstreamBuffers& pBufs,
    const pointField& p
)
...
void CML::cyclicPolyPatch::initUpdateMesh(PstreamBuffers& pBufs)
{
    polyPatch::initUpdateMesh(pBufs);
}


void CML::cyclicPolyPatch::updateMesh(PstreamBuffers& pBufs)
{
...
const CML::edgeList& CML::cyclicPolyPatch::coupledPoints() const
{
...
const CML::edgeList& CML::cyclicPolyPatch::coupledEdges() const
{
    if (!coupledEdgesPtr_)
    {
...
void CML::cyclicPolyPatch::initOrder
(
    PstreamBuffers&,
...
bool CML::cyclicPolyPatch::order
(
    PstreamBuffers& pBufs,
    const primitivePatch& pp,
    labelList& faceMap,
