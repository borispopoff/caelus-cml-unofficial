...
CML::pointField CML::oldCyclicPolyPatch::calcFaceCentres
(
...
CML::pointField CML::oldCyclicPolyPatch::getAnchorPoints
(
    const UList<face>& faces,
    const pointField& points
)
...
CML::label CML::oldCyclicPolyPatch::findMaxArea
(
    const pointField& points,
    ...
    if (debug)
    {
        ...
        for (label zoneI = 0; zoneI < ppZones.nZones(); zoneI++)
        {
            ...
            (
               ...
               /name()+"_zone_"+CML::name(zoneI)+".obj"
            );
...
// right ones.
void CML::oldCyclicPolyPatch::getCentresAndAnchors
(
    const primitivePatch& pp,
...
bool CML::oldCyclicPolyPatch::matchAnchors
(
    const bool report,
    const primitivePatch& pp,
...
CML::label CML::oldCyclicPolyPatch::getConsistentRotationFace
(
...
CML::oldCyclicPolyPatch::oldCyclicPolyPatch
(
    const word& name,
    const label size,
    const label start,
...
void CML::oldCyclicPolyPatch::initGeometry(PstreamBuffers& pBufs)
{
    polyPatch::initGeometry(pBufs);
}


void CML::oldCyclicPolyPatch::calcGeometry
(
...
void CML::oldCyclicPolyPatch::calcGeometry(PstreamBuffers& pBufs)
{}


void CML::oldCyclicPolyPatch::initMovePoints
(
...
void CML::oldCyclicPolyPatch::movePoints
(
    PstreamBuffers& pBufs,
    const pointField& p
)
...
void CML::oldCyclicPolyPatch::initUpdateMesh(PstreamBuffers& pBufs)
{
    polyPatch::initUpdateMesh(pBufs);
}


void CML::oldCyclicPolyPatch::updateMesh(PstreamBuffers& pBufs)
{
    polyPatch::updateMesh(pBufs);
}


void CML::oldCyclicPolyPatch::initOrder
(
    PstreamBuffers&,
    const primitivePatch& pp
) const
...
bool CML::oldCyclicPolyPatch::order
(
    PstreamBuffers&,
    const primitivePatch& pp,
    labelList& faceMap,
