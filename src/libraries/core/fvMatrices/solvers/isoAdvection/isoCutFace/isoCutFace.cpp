...
int CML::isoCutFace::debug = 0;

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

CML::isoCutFace::isoCutFace
(
    const fvMesh& mesh,
    scalarField& f
)
...
void CML::isoCutFace::calcSubFaceCentreAndArea()
{
...
void CML::isoCutFace::subFacePoints
(
    const pointField& points,
    const labelList& pLabels
)
...
void CML::isoCutFace::surfacePoints
(
    const pointField& points,
    const labelList& pLabels
)
...
const CML::point& CML::isoCutFace::subFaceCentre()
{
...
const CML::vector& CML::isoCutFace::subFaceArea()
{
    if (!subFaceCentreAndAreaIsCalculated_)
    {
        calcSubFaceCentreAndArea();
...
const CML::DynamicList<CML::point>& CML::isoCutFace::subFacePoints() const
{
    return subFacePoints_;
}


const CML::DynamicList<CML::point>& CML::isoCutFace::surfacePoints() const
{
    return surfacePoints_;
}


void CML::isoCutFace::clearStorage()
{
    firstEdgeCut_ = -1;
    lastEdgeCut_ = -1;
    firstFullySubmergedPoint_ = -1;
...
CML::scalar CML::isoCutFace::timeIntegratedArea
(
