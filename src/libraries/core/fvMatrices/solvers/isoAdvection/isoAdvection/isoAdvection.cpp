...
void CML::isoAdvection::timeIntegratedFlux()
{
    // Get time step
...
//    scalarField alphaNew = alpha1In_ - fvc::surfaceIntegrate(dVf_);
    ...
    for (label n = 0; n < nAlphaBounds_; n++)
    {
        ...
        {
            if (debug) Info << "Bound from above... " << endl;
        ...
        {
            if (debug) Info << "Bound from below... " << endl;
...
//                    downwind faces
                ...
                if (debug) Info<< "\nNew alpha for cell " << celli << ": "<< alpha1New << endl;
            }
        }
    }

    if (debug) Info << "correctedFaces = " << correctedFaces << endl;
}


CML::scalar CML::isoAdvection::netFlux
(
...
void CML::isoAdvection::syncProcPatches
(
    surfaceScalarField& dVf,
    const surfaceScalarField& phi
)
...
void CML::isoAdvection::checkIfOnProcPatch(const label facei)
{
    if (!mesh_.isInternalFace(facei))
    {
...
void CML::isoAdvection::applyBruteForceBounding()
{
...
void CML::isoAdvection::writeSurfaceCells() const
{
...
void CML::isoAdvection::writeBoundedCells() const
{
...
) const
{
    ...
    const string fName
    (
        "isoFaces_" + CML::name(mesh_.time().timeIndex())
    );
