...
int CML::isoCutCell::debug = 0;

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

CML::isoCutCell::isoCutCell(const fvMesh& mesh, scalarField& f)
:
    mesh_(mesh),
    cellI_(-1),
    f_(f),
...
void CML::isoCutCell::calcSubCellCentreAndVolume()
{
    if (cellStatus_ == 0)
    {
        subCellCentre_ = Zero;
...
void CML::isoCutCell::calcIsoFacePointsFromEdges()
{
    if (debug)
    {
        Pout
    ...
    if (debug) Pout << "Calculated local coordinates" << endl;
    ...
    if (debug) Pout<< "Sorted isoface points by angle" << endl;
}


CML::label CML::isoCutCell::calcSubCell
(
    const label celli,
...
const CML::point& CML::isoCutCell::subCellCentre()
{
...
CML::scalar CML::isoCutCell::subCellVolume()
{
    if (!subCellCentreAndVolumeCalculated_)
    {
        calcSubCellCentreAndVolume();
...
const CML::DynamicList<CML::point>& CML::isoCutCell::isoFacePoints()
{
...
const CML::point& CML::isoCutCell::isoFaceCentre()
{
...
const CML::vector& CML::isoCutCell::isoFaceArea()
{
    if (!isoFaceCentreAndAreaCalculated_)
    {
        calcIsoFaceCentreAndArea();
...
CML::scalar CML::isoCutCell::volumeOfFluid()
{
    if (!subCellCentreAndVolumeCalculated_)
    {
        calcSubCellCentreAndVolume();
...
CML::scalar CML::isoCutCell::isoValue() const
{
    return isoValue_;
}


void CML::isoCutCell::clearStorage()
{
    cellI_ = -1;
    isoValue_ = 0;
...
)
{
    ...
    {
        if (debug) Pout<< "Warning: mag(f1 - f2) < 10*SMALL" << endl;
        return calcSubCell(celli, f1);
    }

    if (mag(a1 - a2) < tol)
    {
        if (debug) Pout<< "Warning: mag(a1 - a2) < tol for cell " << celli << endl;
    ...
    {
        if (debug) Pout<< "Newton did the job" << endl;
        return cellStatus_;
    }
...
void CML::isoCutCell::volumeOfFluid
(
