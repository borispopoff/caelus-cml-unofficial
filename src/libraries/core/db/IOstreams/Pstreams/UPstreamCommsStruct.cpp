...
CML::UPstream::commsStruct::commsStruct()
:
    above_(-1),
    below_(0),
    allBelow_(0),
    ...
    allNotBelow_(nProcs - allBelow.size() - 1)
{
    ...
    {
        FatalErrorInFunction << "problem!" << CML::abort(FatalError);
    }
}
...
bool CML::UPstream::commsStruct::operator==(const commsStruct& comm) const
{
    return
    (
        (above_ == comm.above())
...
CML::Ostream& CML::operator<<(Ostream& os, const UPstream::commsStruct& comm)
{
