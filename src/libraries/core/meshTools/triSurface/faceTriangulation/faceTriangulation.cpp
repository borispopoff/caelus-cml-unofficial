...
const CML::scalar CML::faceTriangulation::edgeRelTol = 1E-6;


// Edge to the right of face vertex i
CML::label CML::faceTriangulation::right(const label, label i)
{
    return i;
}
...
CML::label CML::faceTriangulation::left(const label size, label i)
{
    return i ? i-1 : size-1;
}
...
CML::tmp<CML::vectorField> CML::faceTriangulation::calcEdges
(
    const face& f,
...
void CML::faceTriangulation::calcHalfAngle
(
    const vector& normal,
    const vector& e0,
    const vector& e1,
...
)
{
    ...
    {
        // 3rd or 4th quadrant
        cosHalfAngle = - CML::sqrt(0.5*(1 + cos));
        sinHalfAngle = CML::sqrt(0.5*(1 - cos));
    }
    else
    {
        // 1st or 2nd quadrant
...
CML::pointHit CML::faceTriangulation::rayEdgeIntersect
(
    const vector& normal,
...
// contains point
bool CML::faceTriangulation::triangleContainsPoint
(
    const vector& n,
    const point& p0,
    const point& p1,
...
void CML::faceTriangulation::findDiagonal
(
    const pointField& points,
    const face& f,
...
CML::label CML::faceTriangulation::findStart
(
    const face& f,
    const vectorField& edges,
    const vector& normal
...
// polygons.
bool CML::faceTriangulation::split
(
    const bool fallBack,
    const pointField& points,
    const face& f,
...
// Null constructor
CML::faceTriangulation::faceTriangulation()
:
    triFaceList()
{}
...
CML::faceTriangulation::faceTriangulation(Istream& is)
:
    triFaceList(is)
{}
