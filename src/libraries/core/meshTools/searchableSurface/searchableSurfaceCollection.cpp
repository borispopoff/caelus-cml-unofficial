...
#include "searchableSurfaceCollection.hpp"
...
void CML::searchableSurfaceCollection::findNearest
(
...
CML::searchableSurfaceCollection::searchableSurfaceCollection
(
    const IOobject& io,
...
const CML::wordList& CML::searchableSurfaceCollection::regions() const
{
    if (regions_.size() == 0)
    {
        ...
        forAll(subGeom_, surfI)
        {
            ...
            if (mergeSubRegions_)
            {
                ...
                allRegions.append(instance_[surfI] + "_" + CML::name(surfI));
            }
            else
            {
...
CML::label CML::searchableSurfaceCollection::size() const
{
...
void CML::searchableSurfaceCollection::findLine
(
...
void CML::searchableSurfaceCollection::findLineAny
(
    const pointField& start,
    const pointField& end,
    List<pointIndexHit>& info
...
void CML::searchableSurfaceCollection::findLineAll
(
    const pointField& start,
    const pointField& end,
...
void CML::searchableSurfaceCollection::getRegion
(
    const List<pointIndexHit>& info,
    labelList& region
) const
...
void CML::searchableSurfaceCollection::getNormal
(
    const List<pointIndexHit>& info,
    vectorField& normal
) const
...
void CML::searchableSurfaceCollection::getVolumeType
(
...
void CML::searchableSurfaceCollection::distribute
(
...
void CML::searchableSurfaceCollection::setField(const labelList& values)
{
    forAll(subGeom_, surfI)
    {
...
void CML::searchableSurfaceCollection::getField
(
    const List<pointIndexHit>& info,
    labelList& values
) const
