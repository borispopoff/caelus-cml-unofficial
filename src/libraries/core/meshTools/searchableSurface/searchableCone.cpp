...
CML::tmp<CML::pointField> CML::searchableCone::coordinates() const
{
...
void CML::searchableCone::boundingSpheres
(
    pointField& centres,
...
) const
{
    ...
    {
        radiusSqr[0] = CML::magSqr(point1_-centres[0]) + CML::sqr(radius1_);
    }
    else
    {
        radiusSqr[0] = CML::magSqr(point2_-centres[0]) + CML::sqr(radius2_);
    }
...
void CML::searchableCone::findNearestAndNormal
(
    const point& sample,
...
CML::scalar CML::searchableCone::radius2
(
...
void CML::searchableCone::findLineAll
(
    const searchableCone& cone,
...
void CML::searchableCone::insertHit
(
    const point& start,
    const point& end,
...
CML::boundBox CML::searchableCone::calcBounds() const
{
    // Adapted from
    ...
    min = ::CML::min(min, point2_ - kr);
    max = ::CML::max(max, point2_ + kr);
...
CML::searchableCone::searchableCone
(
    const IOobject& io,
    const point& point1,
...
const CML::wordList& CML::searchableCone::regions() const
{
    if (regions_.empty())
    {
        regions_.setSize(1);
...
void CML::searchableCone::findNearest
(
...
void CML::searchableCone::findLine
(
...
void CML::searchableCone::findLineAny
(
    const pointField& start,
    const pointField& end,
    List<pointIndexHit>& info
...
void CML::searchableCone::getRegion
(
...
void CML::searchableCone::getNormal
(
    const List<pointIndexHit>& info,
    vectorField& normal
) const
...
void CML::searchableCone::getVolumeType
(
