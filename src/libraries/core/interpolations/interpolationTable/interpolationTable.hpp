...
Description
    ...
    has an optional readerType. Default is to read CAELUS format. The only
    other format is csv (comma separated values):

    Read csv format:
        readerType      csv;
        fileName        "$CAELUS_CASE/constant/p0vsTime.csv";
...
template<class Type>
void CML::interpolationTable<Type>::readTable()
{
...
template<class Type>
CML::word CML::interpolationTable<Type>::boundsHandlingToWord
(
...
template<class Type>
typename CML::interpolationTable<Type>::boundsHandling
CML::interpolationTable<Type>::wordToBoundsHandling
(
    const word& bound
) const
{
...
template<class Type>
typename CML::interpolationTable<Type>::boundsHandling
CML::interpolationTable<Type>::outOfBounds
(
    const boundsHandling& bound
)
{
...
template<class Type>
Type CML::interpolationTable<Type>::rateOfChange(const scalar value) const
{
    label n = this->size();
...
template<class Type>
const CML::Tuple2<CML::scalar, Type>&
CML::interpolationTable<Type>::operator[](const label i) const
{
    label ii = i;
    label n  = this->size();
...
template<class Type>
Type CML::interpolationTable<Type>::operator()(const scalar value) const
{
    label n = this->size();
