...
bool CML::argList::bannerEnabled = true;
CML::SLList<CML::string>    CML::argList::validArgs;
CML::HashTable<CML::string> CML::argList::validOptions;
CML::HashTable<CML::string> CML::argList::validParOptions;
CML::HashTable<CML::string> CML::argList::optionUsage;
CML::SLList<CML::string>    CML::argList::notes;
CML::string::size_type CML::argList::usageMin = 20;
CML::string::size_type CML::argList::usageMax = 80;


CML::argList::initValidTables::initValidTables()
{
    argList::addOption
    (
        "case", "dir",
...
void CML::argList::addBoolOption
(
    const word& opt,
    const string& usage
)
...
void CML::argList::addNote(const string& note)
{
    if (!note.empty())
    {
        notes.append(note);
...
void CML::argList::removeOption(const word& opt)
{
...
void CML::argList::noBanner()
{
    bannerEnabled = false;
}


void CML::argList::noParallel()
{
...
void CML::argList::printOptionUsage
(
...
bool CML::argList::regroupArgv(int& argc, char**& argv)
{
    int nArgs = 0;
    int listDepth = 0;
    string tmpString;
...
void CML::argList::getRootCase()
{
    ...
    if (rootPath_.isAbsolute())
    {
        ...
        setEnv("CAELUS_CASE", rootPath_/globalCase_, true);
        setEnv("CAELUS_CASENAME", globalCase_, true);
    }
    else
    {
        ...
        setEnv("CAELUS_CASE", casePath, true);
        setEnv("CAELUS_CASENAME", casePath.name(), true);
    }
}
...
bool CML::argList::setOption(const word& opt, const string& param)
{
    bool changed = false;
...
void CML::argList::printUsage() const
{
    ...
    Info<< nl
        <<"Using: Caelus-" << CML::CAELUSversion
        << " (see www.caelus-cml.com)" << nl
        <<"Build: " << CML::CAELUSbuild << nl
        << endl;
}


void CML::argList::displayDoc(bool source) const
{
    ...
    if (found)
    {
        string docBrowser = getEnv("CAELUS_DOC_BROWSER");
        if (docBrowser.empty())
        {
            docDict.lookup("docBrowser") >> docBrowser;
        }
        // can use CAELUS_DOC_BROWSER='application file://%f' if required
...
bool CML::argList::check(bool checkArgs, bool checkOpts) const
{
    bool ok = true;
...
bool CML::argList::checkRootCase() const
{
    if (!isDir(rootPath()))
    {
        FatalError
