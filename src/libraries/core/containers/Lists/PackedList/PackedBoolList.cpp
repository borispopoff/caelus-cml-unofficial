...
bool CML::PackedBoolList::bitorPrepare
(
...
CML::label CML::PackedBoolList::setIndices(const LabelListType& indices)
{
...
template<class LabelListType>
CML::label CML::PackedBoolList::unsetIndices(const LabelListType& indices)
{
    label cnt = 0;
    forAll(indices, elemI)
    {
...
template<class LabelListType>
CML::label CML::PackedBoolList::subsetIndices(const LabelListType& indices)
{
    // handle trivial case
...
CML::PackedBoolList::PackedBoolList(Istream& is)
:
    PackedList<1>()
{
    is  >> *this;
...
void CML::PackedBoolList::set(const PackedList<1>& lst)
{
...
CML::label CML::PackedBoolList::set(const labelUList& indices)
{
    return setIndices(indices);
}


CML::label CML::PackedBoolList::set(const UIndirectList<label>& indices)
{
    return setIndices(indices);
}


void CML::PackedBoolList::unset(const PackedList<1>& lst)
{
    // operate directly with the underlying storage
    StorageList& lhs = this->storage();
    const StorageList& rhs = lst.storage();
...
CML::label CML::PackedBoolList::unset(const labelUList& indices)
{
    return unsetIndices(indices);
}


CML::label CML::PackedBoolList::unset(const UIndirectList<label>& indices)
{
    return unsetIndices(indices);
}


void CML::PackedBoolList::subset(const PackedList<1>& lst)
{
...
CML::label CML::PackedBoolList::subset(const labelUList& indices)
{
    return subsetIndices(indices);
}


CML::label CML::PackedBoolList::subset(const UIndirectList<label>& indices)
{
    return subsetIndices(indices);
}
...
CML::PackedBoolList CML::operator&
(
