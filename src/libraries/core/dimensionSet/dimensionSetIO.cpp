...
CML::dimensionSet::tokeniser::tokeniser(Istream& is)
:
    is_(is),
    tokens_(100),
    start_(0),
...
void CML::dimensionSet::tokeniser::push(const token& t)
{
...
CML::token CML::dimensionSet::tokeniser::pop()
{
...
void CML::dimensionSet::tokeniser::unpop(const token& t)
{
    ++size_;
...
bool CML::dimensionSet::tokeniser::hasToken() const
{
    return size_ || is_.good();
}


bool CML::dimensionSet::tokeniser::valid(char c)
{
    return
    (
        !isspace(c)
...
CML::label CML::dimensionSet::tokeniser::priority(const token& t)
{
    if (!t.isPunctuation())
    {
        return 0;
...
void CML::dimensionSet::tokeniser::splitWord(const word& w)
{
    size_t start = 0;
...
CML::token CML::dimensionSet::tokeniser::nextToken()
{
    if (size_ == 0)
    {
        token t(is_);
...
void CML::dimensionSet::tokeniser::putBack(const token& t)
{
    if (size_ == 0)
    {
        push(t);
...
void CML::dimensionSet::round(const scalar tol)
{
...
CML::dimensionedScalar CML::dimensionSet::parse
(
        ...
        else if (nextToken.isPunctuation())
        {
            ...
            else if (nextToken.pToken() == '^')
            {
                if (nextPrior > lastPrior)
                {
                    ...
                    ds.value() = CML::pow(ds.value(), exp.value());
                }
                else
                {
                    tis.putBack(nextToken);
...
CML::Istream& CML::dimensionSet::read
(
    Istream& is,
    scalar& multiplier,
    const HashTable<dimensionedScalar>& readSet
    ...
    if (nextToken.isWord())
    {
        ...
        do
        {
            ...
            if (index != string::npos)
            {
                ...
                multiplier *= CML::pow(s.value(), exponent);
            }
            else
            {
                dimensionedScalar s;
...
CML::Ostream& CML::dimensionSet::write
(
    Ostream& os,
    scalar& multiplier,
        ...
        forAll(exponents, i)
        {
            if (mag(exponents[i]) > smallExponent)
            {
                ...
                if (mag(exponents[i]-1) > smallExponent)
                {
                    ...
                    multiplier *= CML::pow(ds.value(), exponents[i]);
                }
                else
                {
                    multiplier *= ds.value();
