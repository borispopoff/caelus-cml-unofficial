...
inline CML::string::string()
{}


inline CML::string::string(const std::string& str)
:
    std::string(str)
{}
...
inline bool CML::string::valid(const string& str)
{
...
template<class String>
inline bool CML::string::stripInvalid(string& str)
{
    if (!valid<String>(str))
    {
        size_type nValid = 0;
...
template<class String>
inline bool CML::string::meta(const string& str, const char quote)
{
    int escaped = 0;
    for (const_iterator iter = str.begin(); iter != str.end(); ++iter)
    {
...
template<class String>
inline CML::string
CML::string::quotemeta(const string& str, const char quote)
{
    if (!quote)
    {
        return str;
...
template<class String>
inline String CML::string::validate(const string& str)
{
    string ss = str;
...
inline bool CML::string::match(const std::string& str) const
{
    // check as string
    return (str == *this);
}
...
inline CML::string CML::string::operator()
(
    const size_type i,
    const size_type n
) const
...
inline CML::string CML::string::operator()(const size_type n) const
{
    return substr(0, n);
}


inline unsigned CML::string::hash::operator()
(
    const string& key,
    unsigned seed
) const
