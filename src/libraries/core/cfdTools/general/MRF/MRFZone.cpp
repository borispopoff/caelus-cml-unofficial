...
CML::vector CML::MRFZone::Omega() const
{
    return omega_->value(mesh_.time().timeOutputValue())*axis_;
}


void CML::MRFZone::addCoriolis
(
...
void CML::MRFZone::addCoriolis(fvVectorMatrix& UEqn, const bool rhs) const
{
    if (cellZoneID_ == -1)
    {
        return;
...
void CML::MRFZone::writeData(Ostream& os) const
{
    os  << nl;
    os.write(name_) << nl;
...
bool CML::MRFZone::read(const dictionary& dict)
{
    coeffs_ = dict;
