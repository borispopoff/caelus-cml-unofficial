...
namespace CML
{
    ...
    <
        CML::meshToMesh::interpolationMethod,
        3
    >::names[] =
    {
        "direct",
...
CML::labelList CML::meshToMesh::maskCells
(
    const polyMesh& src,
    const polyMesh& tgt
) const
...
void CML::meshToMesh::normaliseWeights
(
...
void CML::meshToMesh::calcAddressing
(
...
void CML::meshToMesh::calculate(const word& methodName)
{
    ...
    if (singleMeshProc_ == -1)
    {
        ...
        (
            IOobject
            (
                "newTgt." + CML::name(Pstream::myProcNo()),
...
void CML::meshToMesh::calculatePatchAMIs(const word& AMIMethodName)
{
    if (!patchAMIs_.empty())
    {
        FatalErrorInFunction
...
void CML::meshToMesh::constructNoCuttingPatches
(
    const word& methodName,
...
void CML::meshToMesh::constructFromCuttingPatches
(
    const word& methodName,
    const word& AMIMethodName,
