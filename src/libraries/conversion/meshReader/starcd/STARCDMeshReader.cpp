...
const char* const CML::meshReaders::STARCD::defaultBoundaryName =
    "Default_Boundary_Region";

const char* const CML::meshReaders::STARCD::defaultSolidBoundaryName =
    "Default_Boundary_Solid";

bool CML::meshReaders::STARCD::keepSolids = false;

const int CML::meshReaders::STARCD::starToCaelusFaceAddr[4][6] =
{
...
void CML::meshReaders::STARCD::readToNewline(IFstream& is)
{
    char ch = '\n';
    do
    {
...
bool CML::meshReaders::STARCD::readHeader(IFstream& is, word fileSignature)
{
    if (!is.good())
    {
...
void CML::meshReaders::STARCD::readAux(const objectRegistry& registry)
{
...
\*---------------------------------------------------------------------------*/
void CML::meshReaders::STARCD::readPoints
(
...
)
{
    ...
    mapToCaelusPointId_.setSize(maxId+1);
...
void CML::meshReaders::STARCD::readCells(const fileName& inputName)
{
    ...
    mapToCaelusCellId_.setSize(maxId+1);
    mapToCaelusCellId_ = -1;
    ...
    else
    {
        ...
        while ((is >> lineLabel).good())
        {
            ...
            if (curModelPtr)
            {
                ...
                for (label i=0; i < nLabels; ++i)
                {
                    label pointId = mapToCaelusPointId_[starLabels[i]];
                    if (pointId < 0)
                    {
            ...
            else if (typeId == starcdBaffleType)
            {
                ...
                if (f.size() >= 3)
                {
                    ...
                    mapToCaelusCellId_[starCellId]  = nCells + baffleI;
...
void CML::meshReaders::STARCD::readBoundary(const fileName& inputName)
{
    ...
    //
    // and a conversion table from Star regions to (CAELUS) patchLabels
    //
    ...
    // - this makes it easier to transfer them to an adjacent patch if reqd
    {
        ...
        if (is.good())
        {
            ...
            while ((is >> lineLabel).good())
            {
                ...
                // Build translation table to convert star patch to caelus patch
                label patchLabel = mapToCaelusPatchId[starRegion];
                if (patchLabel == -1)
                {
                    patchLabel = nPatches;
                    mapToCaelusPatchId[starRegion] = patchLabel;
    ...
    // Pass 2:
    //
    if (nPatches > 1 && mapToCaelusCellId_.size() > 1)
    {
        IFstream is(inputName);
        readHeader(is, fileSignature);
        ...
        while ((is >> lineLabel).good())
        {
            ...
            // convert to CAELUS cell number
            if (starCellId < mapToCaelusCellId_.size())
            {
                cellId = mapToCaelusCellId_[starCellId];
            }
            ...
            else
            {
                ...
                if (cellId < cellShapes_.size())
                {
                    ...
                    if (faceLookupIndex.found(index))
                    {
                        ...
                        cellFaceId = starToCaelusFaceAddr[index][cellFaceId];
                    }
                }
                else
                {
...
//
void CML::meshReaders::STARCD::cullPoints()
{
...
bool CML::meshReaders::STARCD::readGeometry(const scalar scaleFactor)
{
...
:
    ...
    cellShapes_(0),
    mapToCaelusPointId_(0),
    mapToCaelusCellId_(0)
{
    readAux(registry);
}
