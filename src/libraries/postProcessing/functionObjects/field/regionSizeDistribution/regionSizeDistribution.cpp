...
void CML::regionSizeDistribution::writeGraph
(
...
void CML::regionSizeDistribution::writeAlphaFields
(
...
CML::Map<CML::label> CML::regionSizeDistribution::findPatchRegions
(
    const polyMesh& mesh,
    const regionSplit& regions
) const
...
CML::tmp<CML::scalarField> CML::regionSizeDistribution::divide
(
...
void CML::regionSizeDistribution::writeGraphs
(
...
) const
{
    if (Pstream::master())
    {
        ...
        forAll(sortedField, i)
        {
            binSqrSum[indices[i]] += CML::sqr(sortedField[i]);
        }
        scalarField binDev
        (
            sqrt(divide(binSqrSum, binCount) - CML::sqr(binAvg))
        );
...
CML::regionSizeDistribution::regionSizeDistribution
(
    const word& name,
...
void CML::regionSizeDistribution::read(const dictionary& dict)
{
    if (active_)
    {
...
void CML::regionSizeDistribution::execute()
{
    // Do nothing - only valid on write
}


void CML::regionSizeDistribution::end()
{
    // Do nothing - only valid on write
}
