...
CML::wordList CML::forces::createFileNames(const dictionary& dict) const
{
...
void CML::forces::writeFileHeader(const label i)
{
    if (i == 0)
    {
        // force data
...
void CML::forces::calcForcesMoment()
{
    initialise();
...
CML::vector CML::forces::forceEff() const
{
    return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


CML::vector CML::forces::momentEff() const
{
