...
const CML::label CML::particle::maxNBehind_ = 10;
...
void CML::particle::stationaryTetReverseTransform
(
    vector& centre,
    scalar& detA,
    barycentricTensor& T
...
void CML::particle::movingTetReverseTransform
(
...
void CML::particle::reflect()
{
    Swap(coordinates_.c(), coordinates_.d());
}


void CML::particle::rotate(const bool reverse)
{
    if (!reverse)
    {
...
void CML::particle::changeTet(const label tetTriI)
{
    if (debug)
    {
...
void CML::particle::changeFace(const label tetTriI)
{
    if (debug)
    {
        Info << "Particle " << origId() << nl << FUNCTION_NAME << nl << endl;
...
void CML::particle::changeToMasterPatch()
{
    if (debug)
    {
        Info << "Particle " << origId() << nl << FUNCTION_NAME << nl << endl;
...
CML::scalar CML::particle::trackToCell
(
    const vector& displacement,
    const scalar fraction
)
...
CML::scalar CML::particle::trackToStationaryTri
(
    const vector& displacement,
    const scalar fraction,
    label& tetTriI
...
CML::scalar CML::particle::trackToMovingTri
(
    const vector& displacement,
    const scalar fraction,
    label& tetTriI
...
CML::scalar CML::particle::trackToTri
(
    const vector& displacement,
    const scalar fraction,
    label& tetTriI
...
CML::vector CML::particle::deviationFromMeshCentre() const
{
...
void CML::particle::prepareForInteractionListReferral
(
...
void CML::particle::correctAfterInteractionListReferral(const label celli)
{
...
CML::label CML::particle::procTetPt
(
