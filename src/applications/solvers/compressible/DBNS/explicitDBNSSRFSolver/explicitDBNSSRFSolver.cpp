...
Description
    ...
    Explicit density-based Navier-Stokes solver for high speed flows
    in single rotating reference frame using the absolute velocity
    formulation. The solution algorithm is based on
    reconstruction-average-evolution (RAE) algorithm that utulizes a
    Rimeann solver for the reconstruction part of the algorithm. No
    flux preconditioning is used so the solver is strictly applicable
    only to high speed flows. Evolution part of the algorithm uses
    low-storage Runge-Kutta mutistage scheme while the averaging is
    performed through a projection on C^0 function space used in
    cell-centered finite vlumes.
